...
namespace Binarios
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            using (var fileStream = new FileStream("arquivo.bin", FileMode.Create, FileAccess.ReadWrite))
            {
                binaryFormatter.Serialize(fileStream, dados);
            }
            //Console.WriteLine(fileStream);

            LojaDeFilmes copiaLojaDeFilmes = new LojaDeFilmes();
            using (var fileStream = new FileStream("arquivo.bin", FileMode.Open, FileAccess.ReadWrite))
            {
                copiaLojaDeFilmes = (LojaDeFilmes)binaryFormatter.Deserialize(fileStream);
            }

            foreach (var filme in copiaLojaDeFilmes.Filmes)
            {
